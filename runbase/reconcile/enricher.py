...
def promote_orphans(conn, groups: list[list[dict]],
    ...
    Each group is a list of orphan dicts for the same date. Each orphan
    becomes its own activity (the review UI's _merge_day() handles
    same-day display merging).
    ...
    for group in groups:
        for orphan in group:
            meta = orphan.get("metadata", {})
            date = orphan["start_date"]
            dist = orphan["distance_mi"] or 0
            dur = orphan["duration_s"] or 0
            avg_pace = dur / dist if dist > 0 else None
            avg_pace_display = format_pace(avg_pace) if avg_pace else None

            workout_name = orphan.get("strava_name") or orphan.get("workout_name")
            workout_type = _map_workout_type(meta.get("workout_type"))
            workout_category = _infer_category(orphan)
            shoe_id = _lookup_shoe_id(conn, orphan.get("gear_id"))
            start_time = meta.get("start_time")

            if verbose:
                print(f"  PROMOTE {date} {dist:.2f}mi \"{workout_name or '?'}\"")
            ...
            cursor = conn.execute(
                ...
                (date, start_time, dist, dur,
                 ...
                 orphan.get("avg_hr"), orphan.get("max_hr"),
                 orphan.get("avg_cadence"),
                 orphan.get("total_ascent_ft") or None,
                 orphan.get("calories") or None,
                 workout_name, workout_type, workout_category, shoe_id),
            )
            ...
            promoted.append({
                ...
                "date": date,
                "distance_mi": dist,
                "pairs": [(orphan["source_id"], activity_id, orphan["id"])],
            })
